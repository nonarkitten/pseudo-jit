...
#define PATH "generated/"
const char *header = 
        "// This file has been autogenerated, do not edit\n"
        "// This file is part of PJIT, see pjit.h for copyright\n"
        "\n"
        "#include \"pjit.h\"\n";
...
int ea_to_len(int ea, int bits, int reg) {
    ea &= 7; reg &= 7;

         if(ea == 5) return 1;
    else if(ea == 6) return 1;
    else if(ea == 7) {
        if(reg == 0) return 1;
        if(reg == 1) return 2;
        if(reg == 2) return 1;
...
void generate_move(char* buffer, int op, int bits) {
    ...
    if(movea) {
        ...
        operands += ea_to_len(op >> 6, bits, op >> 9);
        ...
        if(emit_flags) append(buffer, "\tSET_NZ_%c(temp_imm);\n", emit_reg(bits));    
...
void generate_5_ops(char* buffer, int op) {
    ...
    if((op & 0x0C00) == 0xC00) {
        ...
        if(!emit_write_modify_ea(d, op >> 3, bits, op)) {
            ...
            if(op & 0x0100) {
                if(!emit_flags) append(buffer, "%s += %d; ", d, imm);
                else append(buffer, "%s = add_%s(%s, %d); ", d, t, s, imm);
            } else{
                if(!emit_flags) append(buffer, "%s -= %d; ", d, imm);
...
void generate_6_ops(char* buffer, int op) {
    ...
    if(offset == -1) { emit_read_ea(buffer2, 7, 32, 4); operands = 2; }
    else if(offset == 0) { emit_read_ea(buffer2, 7, 16, 4); operands = 1; }
...
void generate_7_ops(char* buffer, int op) {
    ...
    if(emit_flags) append(buffer, "\tSET_NZ_L(%d);\n", value);    
}
...
int main(void) {
    ...
    static int operands_len[65536] = { 0 };
    ...
    for(int group=0; group<16; group++) {
        sprintf(filename, PATH "pjit_%x_ops.c", group);
        if((file = fopen(filename, "w"))) {
            ...
            for(int ops=0; ops<4096; ops++) {
                ...
                if((errcode = setjmp(error)) == 0) {
                    ...
                    operands_len[(group << 12) | ops] = operands;
                    sprintf(name_buffer, "ops_%04X", (group << 12) | ops);
                    fprintf(file, "void %s(void) { %s}\n", name_buffer, function_buffer);
                } else {
                    sprintf(name_buffer, "vec_%04X", errcode);
                }
    ...
    emit_flags = false;
    for(int group=0; group<16; group++) {
        sprintf(filename, PATH "pjit_%x_opf.c", group);
        if((file = fopen(filename, "w"))) {
            fprintf(file, "%s", header);
            for(int ops=0; ops<4096; ops++) {
                pc_correct = 0;
                ...
                if((errcode = setjmp(error)) == 0) {
                    generator[group](function_buffer, ops);
                    sprintf(name_buffer, "opf_%04X", (group << 12) | ops);
                    fprintf(file, "void %s(void) { %s}\n", name_buffer, function_buffer);
                } else {
                    sprintf(name_buffer, "vec_%04X", errcode);
                }
    ...
    uint32_t used = 0;
    if((file = fopen(PATH "pjit_table_ops.c", "w"))) {
        fprintf(file, "%s", header);
        fprintf(file, "#include \"pjit_table_ops.h\"\n");
        fprintf(file, "\n");
        fprintf(file, "void (*flag_opcodes[65536])(void) = {\n");
        for(int i=0; i<65536; i++) {
            if(opcode_table[i][0] == 'o') used++;
            if((i % 8) == 0) fprintf(file, "\t/* %04X */ ", i);
    ...
    if((file = fopen(PATH "pjit_table_ops.h", "w"))) {
        fprintf(file, "%s", header);
        fprintf(file, "// %d defined opcodes\n", used);
        fprintf(file, "\n");        
        fprintf(file, "extern void (*flag_opcodes[65536])(void);\n");
        fprintf(file, "\n");
        for(int i=0; i<65536; i++) {
    ...
    if((file = fopen(PATH "pjit_opr_len.c", "w"))) {
        fprintf(file, "%s", header);
        fprintf(file, "\n");
        fprintf(file, "uint8_t operand_len[65536] = {\n");
        for(int i=0; i<65536; i++) {
            if((i % 16) == 0) fprintf(file, "\t/* %04X */ ", i);
            fprintf(file, "%d, ", operands_len[i]);
    ...
    // Opcodes with flag-setting
    if((file = fopen(PATH "pjit_table_opf.c", "w"))) {
        fprintf(file, "%s", header);
        fprintf(file, "#include \"pjit_table_opf.h\"\n");
        fprintf(file, "\n");
        fprintf(file, "void (*fast_opcodes[65536])(void) = {\n");
        for(int i=0; i<65536; i++) {
            if((i % 8) == 0) fprintf(file, "\t/* %04X */ ", i);
            fprintf(file, "%s, ", opcode_table[i]);
            if((i % 8) == 7) fprintf(file, "\n");
    ...
    if((file = fopen(PATH "pjit_table_opf.h", "w"))) {
        fprintf(file, "%s", header);
        fprintf(file, "// %d defined opcodes\n", used);
        fprintf(file, "\n");        
        fprintf(file, "extern void (*fast_opcodes[65536])(void);\n");
        fprintf(file, "\n");
        for(int i=0; i<65536; i++) {
            if(opcode_table[i][0] == 'v') continue;
            fprintf(file, "extern void %s(void);\n", opcode_table[i]);
