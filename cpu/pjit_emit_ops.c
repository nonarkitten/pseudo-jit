 ...
 *                  |_|                                                            
 */
#define SPLIT(X) ((((X)&0xF0) << 4) | ((X) & 0xF))
// @brief emit supervisor exception
static void emit_SVC(uint32_t** emit, int exception) {
    *(*emit)++ = NOP;
    *(*emit)++ = 0xEF000000 | exception;
}
// @brief check if we're in supervisor mode, exception if not
static void emit_is_SVC(uint32_t** emit) {
    // r2 should hold SR
    *(*emit)++ = 0xE3120A02; // tst r2, #0x2000
    *(*emit)++ = 0x1F000000 | PRIV; // svcne #privillege
}
// @brief load our X flag into the C flag
static void emit_load_X(uint32_t** emit) {
    // restore X -> C
    *(*emit)++ = 0xE5D05000 | (1 + OFFSETOF(cpu_t, sr)); // ldrb r0, [r5, #cc_offset]
    *(*emit)++ = 0xE270000F; // rsbs r0, r0, #0xF
}
// @brief save out C flag back into the X
static void emit_save_X(uint32_t** emit) {
    // put C back into X, leave everything else
    *(*emit)++ = 0xE5C50000 | (1 + OFFSETOF(cpu_t, sr)); // ldrb r0, [r5, #cc_offset]
    *(*emit)++ = 0x23800010; // orrcs r0, #0x10
    *(*emit)++ = 0x33C00010; // biccc r0, #0x10
    *(*emit)++ = 0xE5C50023 | (1 + OFFSETOF(cpu_t, sr)); // strb r0, [r5, #cc_offset]
}
...
static void emit_src_bit_to_mask(uint32_t** emit) {
    ...
    *(*emit)++ = 0xE3000001; // mov r0, #1
    *(*emit)++ = 0xE1A01011; // lsl r1, r1, r0
    *(*emit)++ = 0xE1110002; // tst r1, r2
}
...
static void emit_68k_to_arm_cc(uint32_t** emit, uint8_t reg) {
    ...
    assert(reg != 0);
    *(*emit)++ = 0xE6FF0F30 | reg; // rbit r0, reg,         cvznx...0000
    *(*emit)++ = 0xE7C30110 | reg; // bfi  r0, reg, #2, #2, cvznx...00nz
    *(*emit)++ = 0xE1A00160;       // ror  r0, r0, #2,      nzcvznx...00
    *(*emit)++ = 0xE128F000;       // msr  CPSR_f, r0
}
...
static void emit_arm_to_68k_cc(uint32_t** emit, uint8_t reg) {
    ...
    reg = ((reg & 0xF) * 0x11000);
    *(*emit)++ = 0xE3C0000F | reg; // bic r2, r0, #0xF
    *(*emit)++ = 0x23800001 | reg; // orrcs r2, #0x01
    *(*emit)++ = 0x63800002 | reg; // orrvs r2, #0x02
    *(*emit)++ = 0x03800004 | reg; // orreq r2, #0x04
    *(*emit)++ = 0x43800008 | reg; // orrmi r2, #0x08
}
// @brief load the 68K SR into reg
static void emit_load_SR(uint32_t** emit, uint8_t reg) {
    *(*emit)++ = 0xE1D050B0 | (reg << 16) | SPLIT(OFFSETOF(cpu_t, sr)); // ldrh r2, [r5, #cc_offset]
    emit_arm_to_68k_cc(emit, reg);
}
// @brief save the 68K SR in reg to cpu_t state
static void emit_save_SR(uint32_t** emit, uint8_t reg) {
    // put reg -> sr
    *(*emit)++ = 0xE1C500B0 | (reg << 12) | SPLIT(OFFSETOF(cpu_t, sr)); // strh r2, [r5, #cc_offset]
    emit_68k_to_arm_cc(emit, reg);
}
// @brief load the 68K CCR into reg
static void emit_load_CR(uint32_t** emit, uint8_t reg) {
    *(*emit)++ = 0xE5D05000 | (reg << 16) | (1 + OFFSETOF(cpu_t, sr)); // ldrb r2, [r5, #cc_offset]
    emit_arm_to_68k_cc(emit, reg);
}
// @brief save the 68K CCR in reg to cpu_t state
static void emit_save_CR(uint32_t** emit, uint8_t reg) {
    // put reg -> sr
    *(*emit)++ = 0xE5C50000 | (reg << 12) | (1 + OFFSETOF(cpu_t, sr)); // strb r2, [r5, #cc_offset]
    emit_68k_to_arm_cc(emit, reg);
}
...
void emit_ALU(uint32_t** emit, ALU_OP_t op, uint8_t regS, uint8_t regD) {
    ...
    uint32_t is_addr_reg = (regD > 5); regD &= 0x1F;
    
    switch(op) {
    // Do nothing
    case ALU_OP_NOP:   opcode = nop(); break;

    // Adders
    case ALU_OP_ABCD:  break;
    case ALU_OP_ADD:   cond = 1; opcode = add_reg(regD, regD, regS, 0); break;
    case ALU_OP_ADDX:  cond = 1; opcode = adc_reg(regD, regD, regS, 0); break;
    case ALU_OP_SBCD:  break;
    case ALU_OP_SUB:   cond = 1; opcode = rsb_reg(regD, regD, regS, 0); break;
    case ALU_OP_SUBX:  cond = 1; opcode = rsc_reg(regD, regD, regS, 0); break;
    case ALU_OP_CMP:   cond = 1; opcode = cmp_reg(regD, regS);          break;

    // Logical
    case ALU_OP_AND:   cond = 1; opcode = and_reg(regD, regD, regS, 0);  break;
    case ALU_OP_EOR:   cond = 1; opcode = eor_reg(regD, regD, regS, 0);  break;
    case ALU_OP_OR:    cond = 1; opcode = orr_reg(regD, regD, regS, 0);  break;

    // Unary
    case ALU_OP_NBCD:  break;
    case ALU_OP_CLR:   break;
    case ALU_OP_NEG:   cond = 1; opcode = rsb_immed(regD, regD, 0); break;
    case ALU_OP_NEGX:  cond = 1; opcode = rsc_immed(regD, regD, 0); break;
    case ALU_OP_NOT:   cond = 1; opcode = mvn_reg(regD, regD, 0); break;

    // Shifts
    case ALU_OP_ASL:   cond = 1; opcode = lsl_reg(regD, regD, regS); break;
    case ALU_OP_ASR:   cond = 1; opcode = asr_reg(regD, regD, regS); break;
    case ALU_OP_LSL:   cond = 1; opcode = lsl_reg(regD, regD, regS); break;
    case ALU_OP_LSR:   cond = 1; opcode = lsr_reg(regD, regD, regS); break;
    case ALU_OP_ROL:   cond = 1; *(*emit)++ = rsb_immed(regS, regS, 31);
    case ALU_OP_ROR:   cond = 1; opcode = ror_reg(regD, regD, regS); break;
    case ALU_OP_ROXL:  break;
    case ALU_OP_ROXR:  break;

    // Multipliers
    case ALU_OP_DIVSW: break;
    case ALU_OP_DIVUW: break;
    case ALU_OP_MULSW: break;
    case ALU_OP_MULUW: break;

    // Bitwise
    case ALU_OP_BCHG:  cond = 0; opcode = eor_reg(regD, regD, regS, 0); break;
    case ALU_OP_BCLR:  cond = 0; opcode = bic_reg(regD, regD, regS); break;
    case ALU_OP_BSET:  cond = 0; opcode = orr_reg(regD, regD, regS, 0); break;
    case ALU_OP_BTST:  cond = 0; break;

    // Miscellaneous
    case ALU_OP_EXTBW: opcode = sxtb(regD, regS, 0); break;
    case ALU_OP_EXTWL: opcode = sxth(regD, regS, 0); break;
    case ALU_OP_SWAP:  opcode = ror_reg(regD, regS, 16); break;
    case ALU_OP_TAS:   break;
    ...
    *(*emit)++ = opcode;
    ...
    if((op != ALU_OP_EXTBW) && (op != ALU_OP_EXTWL)) *(*emit)++ = cmp_immed(regD, 0);

    // fix up our reverse-subtraction
    if((op == ALU_OP_SUB) || (op == ALU_OP_SUBX)) *(*emit)++ = rsb_immed(regD, regS, 0);
...
void emit_BCD_XOP_ALU(uint32_t** emit, uint16_t opcode, ALU_OP_t op) {
    ...
    if(opcode & 0x0004) { sEA |= 0x20, dEA |= 0x20; }
    emit_load_X(emit);
    uint8_t regS = emit_EA_Load(emit, sEA, 1, 1, 0);
    uint8_t regD = emit_EA_Load(emit, dEA, 0, 2, 1);
    emit_ALU(emit, op, regS, regD);
...
void emit_A_LINE(uint32_t** emit, uint16_t opcode) {
...
void emit_F_LINE(uint32_t** emit, uint16_t opcode) {
...
void emit_MOVEQ(uint32_t** emit, uint16_t opcode) {
    int n = (int8_t)opcode;
    if(n > 0) *(*emit)++ = 0xE3A01000 | (n & 0xFFF); // mov r1, value
    else      *(*emit)++ = 0xE3E01000 | (~n & 0xFFF); // mov r1, -value
...
const uint32_t optab_size = sizeof(optab) / sizeof(struct op_details);
 ...
 *                                                                             
 */
void emit_op(uint32_t** emit, uint16_t opcode) {
