...
void emit_ALU(uint32_t** emit, ALU_OP_t op, uint8_t regS, uint8_t regD) {
    ...
    switch (op) {
        ...
        case ALU_OP_ADD:
            cond   = 1;
            opcode = add_reg(regD, regD, regS, 0);
            break;
        case ALU_OP_ADDX:
            cond   = 1;
            opcode = adc_reg(regD, regD, regS, 0);
            break;
        case ALU_OP_SBCD:
            break;
        case ALU_OP_SUB:
            cond   = 1;
            opcode = rsb_reg(regD, regD, regS, 0);
            break;
        case ALU_OP_SUBX:
            cond   = 1;
            opcode = rsc_reg(regD, regD, regS, 0);
            break;
        case ALU_OP_CMP:
            cond   = 1;
            opcode = cmp_reg(regD, regS);
            break;

        // Logical
        case ALU_OP_AND:
            cond   = 1;
            opcode = and_reg(regD, regD, regS, 0);
            break;
        case ALU_OP_EOR:
            cond   = 1;
            opcode = eor_reg(regD, regD, regS, 0);
            break;
        case ALU_OP_OR:
            cond   = 1;
            opcode = orr_reg(regD, regD, regS, 0);
            break;
        ...
        case ALU_OP_NEG:
            cond   = 1;
            opcode = rsb_immed(regD, regD, 0);
            break;
        case ALU_OP_NEGX:
            cond   = 1;
            opcode = rsc_immed(regD, regD, 0);
            break;
        case ALU_OP_NOT:
            cond   = 1;
            opcode = mvn_reg(regD, regD, 0);
            break;
