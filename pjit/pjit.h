...
extern void pjit_jmp(uint16_t *m68k_pc);
extern void pjit_lookup(void);

// This is the size of each "element" in PJIT cache in bits
// A single ARM opcode is 32-bits, so that's "2" while two
// ARM opcopdes would be "3" and four arm opcodes would "4"
#define CACHE_OP_BITS   2
...
static inline uint32_t pjit_cache_reverse(void) {
    register uint32_t lr asm("lr");
    register uint32_t r0 asm("r0");

	uint16_t index = (lr >> (CACHE_OP_BITS + BLOCK_BITS)) & ((1 << INDEX_BITS) - 1);
	uint16_t block = (lr >> CACHE_OP_BITS) & ((1 << BLOCK_BITS) - 1);
    r0 = ((cpu->cache_tags[index] << (1 + BLOCK_BITS + INDEX_BITS))
       | (index << (1 + BLOCK_BITS)) | (block << 1));
