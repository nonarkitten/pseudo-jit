...
uint8_t get_ext(uint16_t opcode) {
    // magic opcode decoder

    // ext_modes holds the ext_t for the opcode
    // source is the low nibble
    // destination is the high nibble
    ...
    // ext_index# holds the opcode decode pattern
    // for the addressing mode
    static const uint8_t ext_index1[20][8] = {
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
    ...
    uint8_t c = ext_index1[b][(opcode >> 3) & 7];
...
__attribute__((naked,used)) void lookup_opcode(void) {
    ...
    // If this is a branch, see if we can jump in-table
    if((opcode & 0xF001) == 0x6000) {
        ...
        case EXT_I16:
            ...
            break;
        case EXT_I32:
            out[i*2+0] = movw(r1, pc[i+2]);
            out[i*2+1] = nop();
            out[i*2+2] = movt(r1, pc[i+1]);
            out[i*2+3] = nop();
            i += 2;
            break;
        case EXT_D8_XAREG:
        ...
        case EXT_I16:
            ...
            break;
        case EXT_I32:
            out[i*2+0] = movw(r2, pc[i+2]);
            out[i*2+1] = nop();
            out[i*2+2] = movt(r2, pc[i+1]);
            out[i*2+3] = nop();
            break;
        case EXT_D8_XAREG:
        ...
        // The opcode table now simply includes a 1:1 copy for each opcode
        out[i*2+0] = cpu->opcode_table[opcode * 2 + 0];

        // Except that if there's a branch-to-stub, we do need to fix-up
        int32_t *op2_addr = cpu->opcode_table + opcode * 2 + 1;
        int32_t branch = *op2_addr;
        if(branch == bx(lr)) {
            branch = nop();
        } else if(branch & 0x0B000000) {
            branch -= calc_offset(op2_addr, out);
        }
        out[i*2+0] = branch;
    }
