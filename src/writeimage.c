...
static uint32_t crc32 (void* buffer, uint32_t len, uint32_t init) {
    ...
    uint32_t crc = init;
    while (len--) {
        crc = (crc << 8) ^ crc32_table[((crc >> 24) ^ *buf++) & 0x00FF];
    }
    return crc;
}
...
void WriteImage(void* start, void* end) {
    ...
    printf("[SPI0] Programming %lu bytes\n", len);
    ...
    // Write load address
    pos = WriteChunk(pos, 0x402F0400, 0);
    ...
    data = rev32(data);
    printf("[SPI0] Read length %lu bytes\n", data);
    if(data != len) goto verify_failed;
    pos += 4;

    ReadSPIBlock(pos, (uint8_t*)&data, sizeof(data));
    data = rev32(data);
    printf("[SPI0] Read load address %08lx\n", data);
...
verify_failed:
    printf("[SPI0] Verify failed at %08lx!\n", pos);
    return;
}